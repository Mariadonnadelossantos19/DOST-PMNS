...
const DostMimaropaReviewModal = ({ 
   ...
   // Fetch application and TNA data when modal opens
   useEffect(() => {
      const fetchData = async () => {
         ...
         try {
            ...
            const appResponse = await fetch(`http://localhost:4000/api/programs/setup/${selectedApplication.applicationId}`, {
               headers: {
            ...
            const tnaResponse = await fetch(`http://localhost:4000/api/tna/list?applicationId=${selectedApplication.applicationId}`, {
               headers: {
                  'Authorization': `Bearer ${token}`
               }
            });
            ...
            if (tnaResponse.ok) {
               const tnaResult = await tnaResponse.json();
               console.log('TNA Data fetched:', tnaResult);
               if (tnaResult.success && tnaResult.data && tnaResult.data.length > 0) {
                  setTnaData(tnaResult.data[0]); // Get the first (and likely only) TNA for this application
               }
            } else {
               console.error('Failed to fetch TNA data');
            }
                        ...
                        <>
                           ...
                           <div className="bg-white rounded-xl border border-gray-200 p-6">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                 ...
                                 <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                                    <p className="text-xs font-medium text-blue-600 uppercase tracking-wide">TNA Status</p>
                                    <p className="text-lg font-bold text-blue-900 mt-1">
                                       {tnaData.status || 'N/A'}
                                    </p>
                                 </div>
